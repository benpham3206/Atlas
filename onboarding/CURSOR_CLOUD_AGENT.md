...
- `tasks/<TASK_ID>.md`, `state/<TASK_ID>.md`, `logs/<TASK_ID>.md`, and `POKE_SUMMARY.md` are
  updated.
...
| Agent does not run tests | Prompt omitted verification or environment failed | Ask it to run the commands from `package.json` and update `logs/<TASK_ID>.md` |
| Agent edits runtime code during smoke test | Goal was too broad | Re-run with "docs-only; do not change runtime code" |
| Poke status is vague | Agent did not update `POKE_SUMMARY.md` | Ask Cursor Cloud to update `POKE_SUMMARY.md` from the task log |
